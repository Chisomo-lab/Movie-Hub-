...
async function fetchMovies(category = '', page = 1, query = '') {
  try {
    ...
    const url = `https://archive.org/advancedsearch.php?q=${searchQuery}&fl[]=title&fl[]=identifier&fl[]=description&fl[]=year&sort[]=downloads desc&rows=20&page=${page}&output=json`;
    ...
    displayMovies(movies.slice(0, 10), trendingMoviesDiv);
