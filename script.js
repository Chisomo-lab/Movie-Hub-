...
// Apply sports filter to any video list
...
async function loadTrending() {
  ...
  let trendingVideos = await fetchVideos('collection:moviesandfilms OR collection:sports OR collection:anime OR collection:animationandcartoons', 10);

  // Apply sports filter globally
...
async function loadHome() {
  ...
  for (const [name, query] of Object.entries(categories)) {
    ...
    let videos = await fetchVideos(query, 4);

    // Apply sports filter if applicable
...
document.getElementById('searchBtn').addEventListener('click', async () => {
  ...
  results = applySportsFilter(results); // sports filter applies globally
