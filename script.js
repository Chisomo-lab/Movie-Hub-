...
async function fetchMovies(category = '', page = 1, query = '') {
  try {
    ...
    const searchQuery = query || (collection ? `collection:(${collection})` : 'mediatype:movies');
...
searchInput.addEventListener("input", () => { currentPage = 1; fetchMovies('', currentPage, searchInput.value); });
