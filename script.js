...
async function fetchMovies(category = '', page = 1, query = '') {
  try {
    ...
    // Build live Archive.org search query
    const searchQuery = query
        ? `${collection ? `collection:(${collection}) AND ` : ''}title:(${query})`
        : (collection ? `collection:(${collection})` : 'mediatype:movies');

    const url = `https://archive.org/advancedsearch.php?q=${encodeURIComponent(searchQuery)}&fl[]=title&fl[]=identifier&fl[]=mediatype&fl[]=year&sort[]=downloads desc&rows=20&page=${page}&output=json`;
