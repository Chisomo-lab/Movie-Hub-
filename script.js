...
async function fetchCategory(cat, page=0){
  ...
  const items = [];
  html.querySelectorAll('div[itemtype="http://schema.org/CreativeWork"]').forEach((el,i)=>{
    if(i>=page*20 && i< (page+1)*20){
      const titleEl = el.querySelector('a[itemprop="name"]');
      if(titleEl){
...
// Load previews for home page (main videos + trending)
async function loadHomeVideos(){
  for(let cat in links){
    const items = await fetchCategory(cat);
    renderVideos(items.slice(0,6),'homeGrid'); // main videos below trending
    if(cat==='sports'){ // trending example
      renderVideos(items.slice(0,6),'trendingSection');
    }
  }
}
loadHomeVideos();
