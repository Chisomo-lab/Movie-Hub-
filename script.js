...
// Load trending (selected first 10 from each category)
...
async function loadCategory(cat){
  ...
  currentPage = 0;
  currentQuery = cat;
  const items = await fetchVideos(LINKS[cat]);
...
window.addEventListener("scroll", async () => {
  if(window.innerHeight + window.scrollY >= document.body.offsetHeight - 200 && !isLoading && currentQuery){
    ...
    renderGrid(items, "movieGrid", true);
    document.getElementById("loading").style.display = "none";
    isLoading = false;
  }
});
