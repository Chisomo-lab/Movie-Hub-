...
async function loadHomeTrending(){
  ...
  renderGrid([...movies, ...cartoons, ...sports, ...anime, ...tv], "trendingContainer");
}
...
async function loadCategory(cat){
  ...
  switch(cat){
    ...
    case "sports": query = "collection:sports"; items = await fetchSports(); break;
    case "anime": query = "collection:anime"; items = await fetchData(query,0,"downloads desc"); break;
    case "television": query = "collection:television"; items = await fetchData(query); break;
    default: query = "collection:moviesandfilms"; items = await fetchData(query);
  }
  currentQuery = query;
